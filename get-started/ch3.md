...
Тут ми бачимо дві визначальні характеристики. Перша: замикання є частиною природи функції. Об'єкти не створюють замикання, їх створюють лише функції. Дргуа: щоб побачити замикання, ви повинні виконати функцію в іншій области видимості, ніж та, де ця функція була визначена.
...
Замикання найчастіше можна спостерігати при роботі з асинхронним кодом, наприклад, з функціями зворотнього виклику(callbacks). Розглянемо:
...
Призначення цього зв’язку прототипу (тобто від об’єкта B до іншого об’єкта A) полягає в тому, що доступ до властивостей чи методів, яких B не має, *делегувався* для обробки об’єкту A. Делегування доступу властивості та методу дозволяє двом або більше об'єктам співпрацювати для виконання завдання.
...
Делегування через ланцюжок прототипів застосовується лише для доступу до значення властивості. Якщо ви натомість призначаєте значення властивості об’єкта, то призначення відбудеться в об'єкті, на йкому виконується, незалежно від того, де цей об’єкт пов’язаний з прототипом.
